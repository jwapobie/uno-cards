...
func _ready() -> void:
	...
	tween.tween_property(self, "position", Vector2(50.0, 0.0), 0.5*Gameplay.anim_time_multiplier).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_IN)
	tween.set_parallel()
...
func pop_off() -> void:
	var tween = create_tween()
	tween.tween_property(self, "position", Vector2(50.0, 0.0), 0.3*Gameplay.anim_time_multiplier).set_trans(Tween.TRANS_QUAD).set_ease(Tween.EASE_OUT)
	tween.set_parallel()
